...
, m_b(NULL)
{
  ...
  m_r = new scalar[m_num_samples];
  m_g = new scalar[m_num_samples];
  m_b = new scalar[m_num_samples];
...
void Colormap::getColorByDensity( const scalar& rho, scalar& r, scalar& g, scalar& b ) const
{
  assert( rho >= 0.0 ); assert( rho <= 1.0 );
  int idx = mathutils::round(((scalar)(m_num_samples-1))*rho);
  getColorByIndex( idx, r, g, b );
}

void Colormap::getColorByIndex( const int& i, scalar& r, scalar& g, scalar& b ) const
{
...
scalar Colormap::getRByDensity( const scalar& rho ) const
{
  assert( rho >= 0.0 ); assert( rho <= 1.0 );
  int idx = mathutils::round(((scalar)(m_num_samples-1))*rho);
...
scalar Colormap::getGByDensity( const scalar& rho ) const
{
  assert( rho >= 0.0 ); assert( rho <= 1.0 );
  int idx = mathutils::round(((scalar)(m_num_samples-1))*rho);
...
scalar Colormap::getBByDensity( const scalar& rho ) const
{
  assert( rho >= 0.0 ); assert( rho <= 1.0 );
  int idx = mathutils::round(((scalar)(m_num_samples-1))*rho);
...
scalar Colormap::getRByIndex( const int& i ) const
{
  assert( i >= 0 ); assert( i < m_num_samples );
  return m_r[i];
}

scalar Colormap::getGByIndex( const int& i ) const
{
  assert( i >= 0 ); assert( i < m_num_samples );
  return m_g[i];
}

scalar Colormap::getBByIndex( const int& i ) const
{
  assert( i >= 0 ); assert( i < m_num_samples );
  return m_b[i];
}
...
void Colormap::generateMatlabJet()
{
	...
	// n = ceil(m/4);
	int n = ceil(((scalar)m_num_samples)/4.0);

	// u = [(1:1:n)/n ones(1,n-1) (n:-1:1)/n]';
	scalar* u = new scalar[3*n-1];
	for( int i = 0; i < n; ++i )
	{
		u[i] = ((scalar)(i+1))/((scalar)n);
	}
	for( int i = 1; i < n; ++i )
	{
		u[n-1+i] = 1.0;
	...
	for( int i = 0; i < n; ++i )
	{
		u[2*n-1+i] = ((scalar)(n-i))/((scalar)n);
	}

	// g = ceil(n/2) - (mod(m,4)==1) + (1:length(u))';
	int* g = new int[3*n-1];
	int addedtoall = ((int)ceil(((scalar)n)/2.0)) - ((m_num_samples%4)==1);
...
void Colormap::generateMatlabHot()
{
	...
	for( int i = 2*n; i < m_num_samples; ++i )
	{
		m_b[i] = ((scalar)(i-2*n+1))/((scalar)(m_num_samples-2*n));
	}
}
...
void Colormap::generateMatlabCool()
{
	// r = (0:m-1)'/max(m-1,1);
	// c = [r 1-r ones(m,1)]; 
	scalar denom = std::max(m_num_samples-1,1);
	for( int i = 0; i < m_num_samples; ++i )
	{
		m_r[i] = ((scalar)i)/denom;
	}
	for( int i = 0; i < m_num_samples; ++i )
	{
		m_g[i] = 1.0 - m_r[i];
...
void Colormap::generateMatlabGray()
{
	...
	for( int i = 0; i < m_num_samples; ++i )
	{
		m_r[i] = m_g[i] = m_b[i] = ((scalar)i)/denom;
	}
}
...
void Colormap::generateMatlabBone()
{
	...
	scalar* accm_r = new scalar[m_num_samples];
	scalar* accm_g = new scalar[m_num_samples];
	scalar* accm_b = new scalar[m_num_samples];
